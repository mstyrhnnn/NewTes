...
import { OrderApprovalStatusEnum } from "src/modules/orders/enums/order.status.enum";
...
export class OrderTaskService {
    ...
    @Cron("1 * * * * *", { name: 'order-task' })
    async handleTask() {
        await this.qontakBeforeSewa(
            new Date(Date.now() + 2 * 60 * 60 * 1000),
            new Date(Date.now() + 2 * 60 * 60 * 1000 + 1 * 60 * 1000)
        );
        await this.qontakAfterSewa();
    }
}